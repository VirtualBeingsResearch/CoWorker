        ...
        : !windowStats ? <div className="admin-usage-state error" role="alert"><TriangleAlert size={18} />{t('运行分析暂不可用')}</div>
          : <>
            <div className="admin-usage-metrics compact">
              ...
              <article className="cost"><CircleDollarSign size={17} /><span>{t('预估消费')}</span><CurrencyCostSummary costs={windowStats.estimated_costs} language={language} limit={1} /><small>{t('定价覆盖 {{rate}} · {{count}} 未定价 Token', { rate: formatCacheRate(windowStats.pricing_coverage), count: formatTokenUnits(windowStats.unpriced_tokens) })}</small><a className="usage-pricing-shortcut" href={pricingHref}>{t('管理定价')}<ArrowUpRight size={12} /></a></article>
