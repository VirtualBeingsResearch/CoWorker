...
export const WECOM_INSTANCE_ID_PATTERN = /^[a-z][a-z0-9_-]{0,31}$/;

export function defaultWeComDisplayName(instanceId: string): string {
  return `WeCom ${instanceId}`;
}
...
export function generateWeComInstanceId(
  ...
  throw new Error('WeCom instance ID space is exhausted');
}
