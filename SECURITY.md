...
- API 默认绑定 `127.0.0.1`；配置通信令牌后，REST 消息、状态查询与 Desktop 通信都要求 Bearer token。通过反向代理暴露服务时，应在代理层终止 TLS，显式设置 `API__HOST`，将 `API__CORS_ORIGINS` 配置为可信浏览器来源，并设置强 `API__COMMUNICATION_TOKEN`。
- `API__DEVELOPMENT_MODE=true` 只会关闭 Coworker API 侧的 REST 消息、状态查询与 Desktop 通信 Bearer 检查；Desktop 侧的 HTTPS 检查由 Desktop 自己的 `security.development_mode` 单独控制。本机 HTTP 调试需要两侧同时开启，且只适用于刻意配置的纯本机环境，绝不能在共享或公开监听地址上启用。
