...
- API 默认绑定 `127.0.0.1`；配置通信令牌后，REST 消息、状态与 Desktop 通信都要求 Bearer token；未携带有效令牌的 `GET /status` 只返回基础生命周期信息。通过反向代理暴露服务时，应在代理层终止 TLS，显式设置 `API__HOST`，将 `API__CORS_ORIGINS` 配置为可信浏览器来源，并设置强 `API__COMMUNICATION_TOKEN`。
