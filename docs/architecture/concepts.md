...
- **潜意识思考**（可选）：设置 `AGENT__SUBCONSCIOUS_THINKING=true` 后，系统自动在后台触发多类反省——**自我审计**、**经验总结**（仅在短期记忆压缩前提炼经验写入长期记忆）、自由发散、技能库审视、宫殿园丁等。各模式的周期触发和压缩前触发节奏都写在 `.coworker/subconscious/*/MODE.md`；内置模式让 `summarize` 在每次压缩前处理即将离开的切片，让 `audit` 在每次压缩前审视完整主线并以每 100 次工具调用兜底，让 `introspect` 在每 3 次压缩且间隔至少 6 小时后运行；`explore` 每 6 小时最多向主线发送一份三点摘要。`meta` 和 `garden` 的时机不变。环境变量只保留总开关、压缩前总结开关和通用 `max_cycles` 兜底。整个过程默认静默运行。
...
短期记忆分为 Agent 自身思考流和按参与者隔离的对话线程。当最近一次完整模型输入达到 `MEMORY__SHORT_TERM_MAX_TOKENS` 预算时，系统按 `MEMORY__COMPRESS_RATIO` 压缩最旧的一段消息——压缩走下文的**多分辨率记忆树**，同时触发 MODE 中到期的压缩前潜意识任务。`summarize` 只接收即将离开原始尾部的切片以提取长期记忆；`audit`、`introspect` 等完整上下文模式接收压缩前的完整主线。该预算是压缩触发点，允许模型输入短暂超过。
