...
短期记忆分为 Agent 自身思考流和按参与者隔离的对话线程。当最近一次完整模型输入达到 `MEMORY__SHORT_TERM_MAX_TOKENS` 预算时，系统按 `MEMORY__COMPRESS_RATIO` 压缩最旧的一段消息——压缩走下文的**多分辨率记忆树**，同时由潜意识在压缩前提取值得长期保留的内容写入 mem0。该预算是压缩触发点，允许模型输入短暂超过。
