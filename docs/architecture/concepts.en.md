...
- **Built-in interaction entries**: File inbox/outbox, REST API, and real-time SSE/WebSocket communication. External Channels connect WeCom, Weixin Claw, Telegram, and Desktop on top.
...
- **Bubble thinking** (optional): With `AGENT__BUBBLE_THINKING=true`, the model can proactively fork independent subtasks from the current context and run them concurrently, then merge their conclusions automatically. The main line and bubbles can communicate in both directions. When a bubble is bound to a `participant_id` (and optionally a `conversation_id`), subsequent matching, unambiguous communication is handed directly to that active bubble; it can reply only to its bound participant. Use `AGENT__BUBBLE_HANDOFF_TRANSPARENCY_PARTICIPANT_MATCHES` to enable visible handoff for selected communication IDs with full-ID globs. Live WebSocket/SSE sessions also enable it by transport by default; use `AGENT__BUBBLE_HANDOFF_TRANSPARENCY_STREAM_TRANSPORTS` to narrow or disable that behavior. The default participant globs match WeCom, Weixin Claw, Telegram, and the Desktop `local` actor, but not Claude or Codex actors. Takeover is delayed until the first real exchange, direct replies are labeled as coming from the Bubble, and completion is sent only for an announced handoff. When the number of active conversations in a sliding window that no bubble has taken over rises above a threshold, the system also injects a parallel hint nudging the model to handle them with bubbles; the window length, threshold, and cooldown between hints are controlled by `AGENT__CONCURRENCY_HINT_WINDOW_SECONDS`, `AGENT__CONCURRENCY_HINT_THRESHOLD`, and `AGENT__CONCURRENCY_HINT_COOLDOWN_SECONDS` (defaults 180 seconds / 2 / 600 seconds). Conversations already owned by an active bubble are excluded, so the hint extinguishes itself once the conversations are delegated. After reaching the cycle limit, a Bubble can continue from its retained context with `bubble_spawn(bubble_id=...)` during the configured grace period.
- **Subconscious thinking** (optional): With `AGENT__SUBCONSCIOUS_THINKING=true`, the system triggers several types of background reflection: **self-audit**, **experience summarization** (extracting durable experience into long-term memory immediately before short-term compression), free association, skill-library review, palace gardening, and more. Periodic and pre-compression cadence both live in `.coworker/subconscious/*/MODE.md`. The built-in modes run `summarize` on the outgoing slice before every compression, run `audit` on the full main line before every compression, run `introspect` after every three compressions, and let `explore` send at most one three-point digest every six hours. `meta` and `garden` timing is unchanged. Environment variables retain only the global switch, the pre-compression summary switch, and a general `max_cycles` fallback. The process stays silent by default.
...
└── data/                    # Runtime data, created or written after startup
    ...
    └── tasks.json           # File used by the task tools
```

## Memory system

Short-term memory is divided into the agent's own thought stream and participant-isolated conversation threads. When the latest complete model input reaches the `MEMORY__SHORT_TERM_MAX_TOKENS` budget, the system compresses the oldest share selected by `MEMORY__COMPRESS_RATIO` through the **multiresolution memory tree** described below and dispatches any due pre-compression MODE work. `summarize` receives only the slice leaving the raw tail, while full-context modes such as `audit` and `introspect` receive the complete main line before compression. The budget is a compression trigger and allows temporary overshoot.
...
- **Time-window recall / unified search**: `query_memory(start=..., end=...)` first uses short-term memory-tree summaries to recall a period; if no summary exists, it falls back to raw logs and generates one. `query_memory(query=...)` searches mem0 long-term memory and returns five results by default. `query_memory(query=..., start=..., end=...)` performs semantic search focused on the specified time window. Inline query output is limited to 3,000 characters. The complete frozen result is written to a temporary Markdown file with a `read_file` path and section line numbers for stable paging and expansion. A call without arguments fails; `query` or both `start` and `end` must be provided.
- **Manual full compression**: `clear_short_term_memory` compresses every remaining live message in the current `primary` stream into the memory tree. It releases context space without deleting memory and first dispatches any due pre-compression MODE work, including `summarize` for long-term-memory extraction. If a tool is currently running, the final `tool_use` is retained to preserve message structure.
...
Long-term memory is managed by **mem0** by default and persists under `MEMORY__DB_PATH` with ChromaDB. Embeddings use the local `sentence-transformers/paraphrase-multilingual-mpnet-base-v2` model by default and load automatically on first use. The backend can be selected with `MEMORY__BACKEND`: `mem0` is the default, and `file` is a minimal file-backed backend (one readable JSON file per memory under `data/memory/long_term/`). Explicit writes such as `manage_memory(write)` already contain model-distilled final memories, so Coworker skips the second LLM extraction and saves them directly. Before an explicit write, Coworker searches for and skips an existing memory with exactly the same body. Semantically similar text with a different body remains a separate memory until a later maintenance flow merges it explicitly.

**Automatic recall**: Whenever a user message arrives, the system semantically searches long-term memory and injects results above the relevance threshold under the `[automatic recall]` marker. IDs of memories already recalled or written through `manage_memory(action="write")` are stored in `Message.recalled_memory_ids` and persisted in snapshots, preventing duplicate injection in the same session, including after a restart.
...
- When the main line encounters a specialized task matching a palace, it spawns a bubble with `bubble_spawn(palaces=[...])`. A specialized execution usually sets `fresh_start=true` for clean domain context, and one bubble can attach multiple palaces.
