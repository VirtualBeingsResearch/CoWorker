...
**mem0** manages long-term memory and persists it under `MEMORY__DB_PATH` with ChromaDB. Embeddings use the local `sentence-transformers/paraphrase-multilingual-mpnet-base-v2` model by default and load automatically on first use. Raw conversations go through the batch path, where mem0 calls an LLM to extract and semantically merge durable information. Explicit writes such as `manage_memory(write)` already contain model-distilled final memories, so Coworker skips the second LLM extraction and directly embeds and stores them. Before an explicit write, Coworker searches for and skips an existing memory with exactly the same body. Semantically similar text with a different body remains a separate memory until a later maintenance flow merges it explicitly.
