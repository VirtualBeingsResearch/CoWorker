...
| `data/admin_config.json` | 管理员令牌及管理页面保存的配置，可能包含 API Key 和自定义 System Prompt 模板 |
...
它们的安全性由操作系统权限、磁盘加密和最小权限账户决定。配置导出包会包含运行数据和密钥，
等同凭据文件。
...
- 自定义 System Prompt 正文属于管理员信任输入，会原样进入系统提示词并发送给模型 Provider；
  粘贴进模板的外部内容和秘密同样会以原文离开本机。
- `visual_analyze` 会把选中的图片或视频发送给配置的视觉模型服务。
- 搜索工具会发送查询词；浏览器工具会访问目标网站，并受到该网站的日志、Cookie 和会话
  策略约束。
- 企业微信、Telegram、Desktop 桥接及其他通信或 MCP 集成会向对应服务传输消息、附件和协议元数据。
...
若数据不能交给某个外部服务，为 Coworker 配置该服务或让 Agent 读取相关文件都会把数据
送出这条边界。使用自托管模型只能改变模型这一条边界，不会自动限制搜索、浏览器或其他集成。

## 执行与网络边界

- Coworker 不是安全沙箱。命令、文件和浏览器工具以启动进程的系统用户权限运行；这些工具
  能触及的范围由运行账户的权限和挂载的目录决定。
- 网页、消息、附件、技能、记忆和模型输出都属于不可信输入，可能包含提示注入或恶意内容。
- API 默认绑定 `127.0.0.1`。管理员令牌保护管理 API，但当前 v0.x 并非每个路由都具备完整的
  多租户授权边界；直接暴露 8000 端口会绕过这层回环绑定保护。远程部署要求 TLS、可信 CORS
  来源、强通信令牌和额外的网络访问控制。详见 [安全策略](../../SECURITY.md)。
- 从公网访问 Desktop 的路径是[自托管中继（Relay）](../operations/relay.md)，而不是直接公开的
  Coworker 8000 端口。Relay v1 是单节点公网安全边界，不是多租户隔离平台或通用代理。
...
`credentials/`、桌面应用数据或 Docker 卷。完整移除因此意味着逐项处理这些位置及
`data/_backups/`，并在删除前确认不再需要恢复。容器删除不会移除绑定目录和命名卷。
