...
当前微信 Claw 入站会提取文本和语音转写；图片、文件和视频在访问控制通过后从微信 CDN 下载、用消息自带的 AES key 解密，并作为附件交给 Agent（消息文本中仍保留对应的占位说明）。语音消息不经过 CDN 下载，仅使用其转写文本。单个附件的下载上限由 `weixin.max_download_mb`（环境变量 `WEIXIN__MAX_DOWNLOAD_MB`，默认 200 MB，可在 `/admin` 微信设置页调整）配置；超过上限或下载、解密失败时，入站内容会标明对应附件未随消息交付，底层错误只写入运行日志，不会向 Agent 暴露下载 URL、AES key 或异常详情，当前消息也不会被丢弃。普通出站目前发送文本。
