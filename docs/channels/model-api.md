...
**Agent 也可以在对话中直接管理令牌**（persona 工具）：`issue_token` 为人物签发令牌（明文一次性返回，由 Agent 转交对方保存）、`revoke_token` 撤销令牌、`list_tokens` 列出人物名下令牌，带 `key` 时可取回该令牌明文（对方丢失令牌时重新交付）。签发与撤销无需管理员确认，但**启用权归管理员**：模型接口未启用时签发会被拒绝（提示管理员在「配置 → 模型接口」中启用），撤销与查看不受此限制；Agent 只能操作已绑定到目标人物地址的令牌。Agent 的签发与撤销会以 `source: "agent"` 写入管理审计日志（`admin_audit.jsonl`），在审计视图中可见。
...
每个令牌对应一个参与者（participant），地址由令牌键名派生：`api:<键名>`（如 `api:alice`）。通过 `.env` 预置的令牌尚未绑定人物时，Agent 会看到未知的 `api:` 地址，可像其它信道一样在对话中用 persona 工具手动绑定，或直接用 `issue_token` 为已有人物重新签发。
...
- 调用方场景材料（system prompt + tools）原样落盘保存为文档，注入会话的只是一份简短说明；Agent 需要原文时可用读文件工具自行查看。
