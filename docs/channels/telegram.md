...
聊天发送文本或附件。每个 Bot 都有独立的凭据、机器人 API 地址、长轮询 offset 和联系人集合，
...
每条 Telegram 入站内容都会以简短的信道自有头明确标注 `私聊`、`群聊` 或 `频道`，不改变
Coworker 的通用入站消息外壳，也不会重复外壳已有的 `Telegram` 来源名。群组中，Coworker
还会在正文前保留发送者 ID、用户名和显示名，避免把群组 participant 误当成单个成员。
...
|---|---|---|
| 正文 | 文本、caption | 保留原文 |
| 可下载附件 | 图片、文档、视频、视频消息、音频、语音、动画 | 显示类型并把一个附件下载给 Coworker |
| 可读摘要 | 贴纸、联系人、位置、地点、投票、骰子、故事 | 保留关键字段，不下载底层媒体 |
| 消息关系 | 同聊天引用、局部 quote、外部引用、故事引用、转发来源 | 在正文前显示来源与引用摘要；引用最长 1000 字符 |
...
- 自定义 API 可发送但不能下载：确认 `api_base_url` 是服务根地址，且反向代理同时转发
  `/bot...` 与 `/file/bot...` 路径；若本地机器人 API 服务器以 `--local` 启动，还要共享文件
  路径并启用 `local_mode`。
