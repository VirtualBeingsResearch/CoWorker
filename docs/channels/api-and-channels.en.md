...
Coworker exposes the running lifeform as an OpenAI-compatible model: `GET /v1/models` returns `coworker`, and `POST /v1/chat/completions` is the inbound entry. Cursor, Open WebUI, or an SDK can call it as a model. This is an ordinary Channel, not a second Agent, and it is not forwarded through Relay.

The arriving Bearer maps to participant `openai:{short_name}`. The primary `API__COMMUNICATION_TOKEN` is `openai:api`; Desktop copy and Relay crypto use that primary token. `communicate` on `openai:control` (`extra.action` of `issue` / `rotate` / `revoke` / `list`) operates on extras only; it does not issue, rotate, or revoke the primary token. Short names match `[a-z][a-z0-9_-]{0,31}`; `api` and `control` are rejected. `issue` may also persona-bind with `person_id` or `person`; the secret appears once in the issue/rotate result. `openai:{short_name}` is a 1:1 bindable address; persona bind uses that address without `conversation_id`. Unbound extras can still send inbound mail. `conversation_id` is a window under that address (body field, `X-Coworker-Conversation-Id`, or a fingerprint of the first system plus first user); `communicate` and bubbles reuse that window ID, and it is not another participant.

Per-request `tools` / `system` travel with that inbound letter. The main thread and bound bubbles invoke client functions through the stable `call_client_tool`; client schemas are not registered on `get_schemas()`. `call_client_tool` dispatches without waiting: the HTTP turn returns the caller's original `tool_calls`, and the client result arrives as a later inbound letter. A later tool round may resend earlier `tool` messages; only the currently pending call IDs are consumed. `communicate` completes the HTTP turn (`stop`). The waiter timeout is `API__COMPAT_TIMEOUT_SECONDS` (default 180). During first-run setup, `/v1/*` returns JSON `503` rather than a 303 HTML redirect to `/admin`.
