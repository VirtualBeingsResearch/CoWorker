...
[API 与通信入口](api-and-channels.md)。当前 v0.x 没有企业级多租户授权边界，直接暴露到
公网的实例仅由「认证范围」一节列出的默认校验保护。
...
令牌时回退使用管理员令牌。长期使用通常显式设置独立的 `API__COMMUNICATION_TOKEN`。
...
`sender_id` 是持续对话隔离边界的一部分，隔离边界依赖稳定、可审计且专属于对应参与者的 ID。附件
...
配置与用量。未配置通信令牌或携带有效 Bearer 时返回完整快照。字段会随可用模块增加，容忍未知字段的
客户端在不同版本间保持兼容。需要稳定审计的集成通常保存原始响应和 Coworker 版本。
...
- SSE 每 15 秒发送注释心跳；缓冲响应的代理会延迟事件送达。
...
出站事件包含正文，并可能包含结构化 `extra`，例如 Bubble 接管状态。接管状态由
`extra.bubble` 承载，本地化提示文字不是稳定接口。
...
| 状态 | 处理 |
|---|---|
| `400/422` | 由请求、模型或协议字段引起，原样重试返回同样错误 |
| `401/403` | 对应令牌、认证范围或信道访问列表的问题；完整 Authorization 值是凭据，进入日志即泄露 |
| `404` | 资源不存在，或版本、功能未启用 |
| `409` | 当前已有任务或连接；状态可通过 `GET /status` 查询 |
| `503` | Agent、Channel 或令牌尚未就绪；通常以退避间隔重试直到就绪 |

普通 `/messages` 没有通用幂等键。只有 Desktop 协议使用 `message_id` 做有界去重；重试会在
自定义集成中重复产生有副作用的用户消息。

## 管理与发布接口

`/api/admin/*` 和 `/api/desktop-updates/*` 会修改配置、恢复状态或发布制品，并按同版本官方
管理端的契约设计。常规操作的入口是 Web 界面；相关背景见
[Web 管理后台](../guides/README.md) 和[数据与信任边界](../architecture/data-boundaries.md)。
