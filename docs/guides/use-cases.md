...
这些场景展示如何组合现有能力，不是要求 Coworker 自动获得更多权限；关键操作的批准仍经过
人工复核。
...
整份项目资料固定到短期上下文会长期占用上下文；流程写 Skill，事实进入记忆。
...
- 重要决策在人工确认后再写入长期记忆；
- 用 Channel 或 Desktop 传递结果；对话隔离只分开上下文，不提供权限边界。
...
- `/api/admin/*` 是同版本 Web 管理端的实现契约，不是长期公共 API。
