...
记忆、任务、模型、身份、人物关系、能力内容、远程连接和诊断记录。它是管理面，不是面向
公网的多租户控制台。
...
初始化完成后，全局导航收敛为“生命总览、运行工作台、配置工作台、人物与身份、高级管理”
五个工作台。桌面和窄侧栏可单击进入工作台，相关功能通过顶栏紧凑页签切换；再次进入工作台
会回到本次会话上次使用的页签。手机端使用五项底栏，并可从顶部直接选择全部 12 个功能页。
原有的 `?section=` 深链接继续有效。
...
- Agent 是活跃、休息还是等待事件；
- 今日、近 7 日和累计的模型 Token、输入、输出、缓存、调用与来源拆分；
...
“用量分析”是生命总览工作台内的第二个页签，用于从摘要继续钻取，而不会在左侧增加新的
全局菜单项。它提供：

- 今日、近 7 日、近 30 日和累计四个窗口；
- 今日对昨日、近 7 日对前 7 日、近 30 日对前 30 日的等长周期比较；
- 最近 30 天的输入、输出 Token 日趋势；
- Token 追踪覆盖率、缺失调用和本地估算调用提示；
- Provider / 模型、主线与 Bubble 等职责来源、工具调用排行；
- 当前管理员报告的 JSON 和每日汇总 CSV 导出。

![Coworker Web 管理后台的用量分析](../assets/screenshots/admin-usage-zh.png)

<p align="center"><sub>用量分析 · 对比周期趋势，并定位模型、职责来源与未追踪调用。</sub></p>

公开状态页继续使用不含日明细的紧凑快照；30 天趋势和导出只由已认证的管理员接口提供。
...
![Coworker Web 管理后台的运行工作台](../assets/screenshots/admin-operations-zh.png)

<p align="center"><sub>运行工作台 · 在同一上下文中切换任务、记忆和诊断。</sub></p>
