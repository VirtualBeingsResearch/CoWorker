...
- 它进入 Git、截图或聊天即等同于泄露管理员权限；
- 不受信任的浏览器扩展能读取页面上显示的令牌和配置；
- `/admin` 与 Coworker 的 `8000` 端口直接暴露到公网时失去回环绑定保护；
- Desktop 日常通信通常使用独立的 `API__COMMUNICATION_TOKEN`。

页面显示的确认名称来自当前身份。完整恢复、短期记忆全量压缩、重启等破坏性或状态改变
操作会要求输入确认名称；这是一道防误操作确认，不替代权限和备份。

## 第一次进入

尚未初始化时，后台只显示首次设置流程。完成 Provider、模型、API Key 和启动模式设置后
保存，Coworker 会重启；运行语言、输出 Token 等完整参数可在高级初始化中调整。详细步骤见[首次运行](../getting-started/README.md)。
...
`pending` 任务经常表示正在等待消息或定时器，并不自动等同于卡死。“诊断与审计”、日志和
最近一次成功活动是判断故障的依据。
...
全量压缩会调用模型并改变短期上下文结构，执行依赖可用的模型；回溯在后台运行，离线回溯
命令与它同时运行会争用快照文件。删除长期记忆不可通过普通页面撤销，先确认它不再需要。

固定上下文适合少量、稳定且必须持续可见的信息；大段原始资料固定后会长期占用上下文。
...
Provider。fallback 按从上到下的接棒顺序生效；修改时通常保持该顺序并移除已失效的 Provider。

首次真实调用可能产生费用，也可能暴露模型或网关能力不兼容。更改前通常先确认 Provider 的
数据边界、模型工具调用能力和成本。
...
出站允许/拒绝规则。`stream`、`desktop`、`wecom`、`weixin` 和 `telegram` 始终显示在信道选择器中，
扩展 Channel 可以按注册名添加。
...
Desktop development mode 对应同机本地调试，远程连接的传输安全来自 HTTPS 或 Relay 配置。
完整环境变量语义见 [配置与模型](../operations/configuration.md)。
...
一次。展开变量卡片可以预览当前运行实例的完整区段与仅正文内容，并选择插入完整变量或
`_CONTENT` 正文变量；同一区段两种形式不能同时使用。编辑器带同步行号、空白行标记和空白行
计数，便于发现意外增加的分隔行。模板保存到管理覆盖层后不会热更新，状态区会明确区分当前运行 Prompt 和等待安全重启
的期望模板；“恢复继承”会回到 `.env` 等启动配置，保存标准空值则明确使用产品标准模板。

“当前 System Prompt”仍是只读的实际缓存版本，不包含工具 schema、短期上下文或本轮消息。
它适合验证身份和系统配置是否已经进入提示词，而不是完整请求审计。完全替换会移除模板
没有引用的 Identity、语言策略、Skill、Palace 和 Channel 指引。

管理员界面里其他按行或源码型编辑器也使用相同的行号、空白行标记和统计，包括模型失败
降级链、JSON 配置、人物逐行备注、信道批量规则，以及 Skill、Palace、潜意识模式的源码。
人格、任务详情、发布说明等普通自然语言长文本仍使用常规编辑器。

### 通信录

通信录把同一真人在不同信道中的地址绑定为一个人物关系。可以新建、改名、合并人物，查看
画像框架并维护个性化备注。合并前通常核对双方地址；删除人物会移除关系记录，不能从普通页面
撤销。
...
文件；稳定元数据和工具名始终来自原文件。删除资产前检查是否仍被 Prompt、其他资产或日常流程
引用。

Skill 与 Palace 会进入模型上下文；来自网页、消息或第三方资料的内容未经审阅就写入时，
恶意指令可能成为持久提示注入。
