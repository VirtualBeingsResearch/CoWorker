...
哪些领域能力”，潜意识模式负责“何时在后台反思”。它们都可能进入模型上下文，通常按
代码变更的方式审阅、测试和版本管理。
...
`name` 必填且全局唯一；`description` 决定 Agent 何时发现它。正文通常写明触发条件、输入、
步骤、停止条件、失败恢复和不得执行的动作。令牌、个人数据或未经审阅的网页指令写入
Skill 后会进入模型上下文并持久保存。
...
`memory_tags` 用于召回和写回长期事实。正文保持薄而稳定，只提供心智模型、易错点和
...
删除或重命名前搜索其他资产对其名称和标签的引用。来自第三方的能力内容属于不可信输入，
未经审阅保存会把提示注入持久化到模型上下文。
