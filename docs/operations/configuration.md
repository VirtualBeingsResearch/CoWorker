...
| `API__COMPAT_TOOL_RESULT_FOLD_CHARS` | `2000` | 单条客户端工具结果的内联字符上限（1–1000000）；超出后完整结果折叠到 `data/openai/detail/`，入站消息只保留预览与 `read_file` 路径 |
...
| `WEIXIN__MAX_DOWNLOAD_MB` | `200` | 微信 Claw 入站图片/文件/视频单个附件的下载上限（MB），超过时跳过下载并在消息中标注附件大小与该上限；范围 1–10000 |
...
管理端「生命总览」还提供「暂停」。暂停会热生效并持久化 `agent.paused` 配置，主循环
随即停靠：不再处理新消息、不再按空闲超时自唤醒，新消息保留在收件队列，恢复后一起
处理；Bubble 并行思考与潜意识后台不受影响。暂停状态在重启后保持，直到在管理端点击
「恢复运行」或调用 `POST /api/admin/resume` 清除该状态。
