...
`pending` 常表示等待消息或定时器，不等于故障。是否真的卡死取决于等待位置、最近成功活动
和错误是否重复。
...
再发送一条不会触发高风险工具的测试消息，确认入站、模型和回复路径。`GET /status` 与
Docker healthcheck 无副作用；`POST /messages`、`POST /backups/restore` 等接口会产生模型
费用或改变状态，不适合作为探针。

## 用量与成本

已配置通信令牌并携带 Bearer 的 `GET /status` 会返回 `usage_stats` 字段，提供 today、last_7_days 和 lifetime 窗口，并
按模型、Provider/模型和 main、summary、vision、bubble、subconscious、long_term 等 scope 拆分；这个普通
状态接口只返回用量，不返回金额。
...
问题记录通常包括发生时间、时区、participant、Channel 和第一个错误。日志与配置导出包含
令牌、密钥、消息正文、附件、个人路径、微信二维码和 Relay 配对材料等敏感信息；完整配置
导出包集中包含全部配置，同样属于敏感材料。

`data/logs/channel_traffic.jsonl` 是管理端“消息流量”的元数据来源：不含消息正文、附件内容或
凭据，但包含可能敏感的 participant ID。文件达到 10 MiB 后轮转并保留 6 份备份；调整整体
日志备份策略时，这些文件也属于访问控制、保留和清理的对象。
...
- 每日：持续失败任务、异常用量、磁盘余量和待处理闹钟。
- 每周：Provider/fallback、备份结果、离线 participant 和长期任务。
- 每月或重大升级前：恢复演练、能力内容审查、版本和容量趋势记录。

典型告警至少覆盖：进程/健康检查连续失败、磁盘空间不足、错误任务持续增长、Relay 不可达
和备份过期。当前项目不直接提供 Prometheus 指标；外部监控通常轮询轻量状态和进程/磁盘
信号，包含敏感正文的日志不是默认指标来源。
