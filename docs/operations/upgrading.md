...
本页适用于已经保存身份、记忆、任务或客户端连接的实例。升级包含的不只是代码更新：记录
版本、备份状态，并检查本次版本是否包含数据格式或协议变化，都是升级流程的一部分。
...
5. 保留当前可启动的提交或镜像标签；浮动标签会随发布移动，不能作为固定回滚点。

> [!WARNING]
> 强制同步、重置或覆盖代码会丢弃工作区中未确认的修改，包括 Coworker 自己在当前
> checkout 中留下的分支和提交。
...
容器手动配置。想用自己的仓库管理后续修改时，常见做法是将它设为 `origin`，将
...
如果默认分支不是 `main`，请替换为实际分支名；对已存在的同名 remote 执行
`git remote add` 会直接报错。仓库 URL 不包含 Token、密码或私钥。公开仓库拉取无需额外
凭据；私有仓库或推送操作需要事先在容器或运行账户中配置专用、最小权限的
Git 凭据，发送到聊天中的凭据会留在会话记录里。

定期同步的指令通常会写明频率、要维护的本地分支、上游分支和是否推送；缺少这些信息时，
任务触发时使用的是当时的当前分支。

## 让搭档升级自己

对于源码 checkout，常见做法是先让 Coworker 自己检查并执行升级。她可以使用文件、代码和命令
...
Provider 注册”，不能证明所有集成测试、数据迁移或外部服务都正常；备份、差异审阅和
相关测试在它的覆盖范围之外。
...
Git bundle 快进它；本地修改、提交、其他分支和分叉历史都会保留。`coworker-before-upgrade`
与备份构成回滚路径，在新容器验证通过前是仅有的回退手段；两个容器共用同一状态卷，
旧镜像回退的前提是数据格式兼容，必要时恢复升级前备份。
...
迁移目录包含管理员令牌、模型密钥、对话和附件，与这份敏感度匹配的位置是只有运行账户
可读的目录。验证新容器完整后再处理 `../coworker-data-before-compose`；在此之前它是
数据的唯一副本。
...
- 降级版本不能假定能够读取新版本写入的数据。
- 记忆树升级后默认只从新的压缩事件继续增长；若需要把历史日志回填为多分辨率树，
  在管理后台运行回溯，或使用 `POST /backfill_tree`。回溯会产生模型调用。
- 更换 embedding 模型会改变长期记忆向量空间。对已有记忆的实例，版本说明没有提供明确
  重建流程时，直接切换会让旧数据与新向量空间不匹配。
...
- Relay v1 的 Go Relay、Python Coworker 和 Rust Desktop 使用协议版本 `1`。协议或密钥
  派生变化后三者需要协同升级，详见 [Relay v1 协议](relay-protocol.md)。
- Desktop 与 Coworker 连接会协商协议版本；旧客户端可能继续本地工作，但无法连接不兼容
  的 Coworker 或 Relay。
- Desktop 自动更新失败不会删除当前安装版本。签名或平台资产不完整的更新会让旧版本保持安装状态。
...
若新版本已经写入不兼容数据，恢复路径是停止服务、保留故障现场，再恢复升级前的完整
备份；删除 `data/` 或 Docker 卷会永久移除运行数据，不属于回滚路径。
