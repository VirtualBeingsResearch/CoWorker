...
| `API__DEVELOPMENT_MODE` | `false` | Kept for compatibility; it no longer disables any API communication Bearer check, and Desktop-side HTTPS enforcement remains controlled by Desktop `security.development_mode` |
| `API__COMMUNICATION_TOKEN` | Empty (administrator-token fallback) | Bearer token for production communication; protects Desktop traffic, ordinary REST messages, and the full `/status` snapshot when set, and should be configured separately to isolate permissions |
