...
| `LLM__TOOL_CHOICE_REQUIRED` | `true` | Require main-line and bubble models to call at least one tool when tools are available; set to `false` for APIs incompatible with required tool choice. The main loop still immediately reminds when no tool is called and enters rest after 5 consecutive reminders |
