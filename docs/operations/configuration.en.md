...
| `API__COMMUNICATION_TOKENS` | `{}` | Extra OpenAI-channel communication tokens: JSON object of short name → secret. Short names match `[a-z][a-z0-9_-]{0,31}` and cannot be `api` or `control`. She issues them through `openai:control`; the administration console is backup only. Saving refreshes auth immediately. Extra tokens do not participate in Relay |
