...
| `API__METRICS_ENABLED` | `true` | Whether to expose `GET /metrics` (Prometheus text format) on the API port and collect runtime metrics; when `false`, the endpoint answers 404 and counting stops. Applies after a restart |
