...
| `MEMORY__SUMMARY_BUDGET_RETRIES` | `3` | Maximum retries after a compression summary first exceeds the node budget (first attempt excluded); summaries still over the acceptance limit (budget × (1 + tolerance)) are then truncated |
| `MEMORY__SUMMARY_BUDGET_TOLERANCE` | `0.10` | Summary budget tolerance: a summary is accepted — no retry, no truncation — when its tokens are within node budget × (1 + tolerance) |
...
| `API__COMMUNICATION_TOKEN` | Empty (administrator-token fallback) | Bearer token for production communication; when explicitly set it protects Desktop traffic, ordinary REST messages, WebSocket/SSE connections, the runtime log stream, and the full `/status` snapshot. Saving through the management console applies immediately; `.env` changes require a restart. Configure it separately to isolate permissions |
| `API__COMMUNICATION_TOKENS` | `{}` | Extra OpenAI-channel communication tokens: JSON object of short name → secret. Short names match `[a-z][a-z0-9_-]{0,31}`; `api` and `control` are rejected. Issued through `openai:control`; the administration console can list, copy, revoke, or add. Saving refreshes auth immediately. Desktop copy and the Relay tunnel identity still use the primary token; extras authenticate direct and inner-Relay `/v1/*` the same way |
| `API__COMPAT_TIMEOUT_SECONDS` | `180` | Seconds the OpenAI-compatible `chat/completions` waiter waits for `communicate` or client-tool results (1–3600) |
| `API__COMPAT_TOOL_RESULT_FOLD_CHARS` | `2000` | Inline character cap for a single client tool result (1–1000000); longer results are folded to `data/openai/detail/` and the inbound message keeps only a preview plus a `read_file` path |
