...
| `API__COMMUNICATION_TOKENS` | `{}` | Extra OpenAI-channel communication tokens: JSON object of short name → secret. Short names match `[a-z][a-z0-9_-]{0,31}`; `api` and `control` are rejected. Issued through `openai:control`; the administration console can list, copy, revoke, or add. Saving refreshes auth immediately. Extra tokens do not participate in Relay |
