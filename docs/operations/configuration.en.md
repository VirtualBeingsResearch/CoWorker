...
| `API__COMMUNICATION_TOKEN` | Empty (administrator-token fallback) | Bearer token for production communication; when explicitly set it protects Desktop traffic, ordinary REST messages, the runtime log stream, and the full `/status` snapshot, and should be configured separately to isolate permissions |
