...
| `API__COMMUNICATION_TOKEN` | Empty (administrator-token fallback) | Bearer token for production communication; when explicitly set it protects Desktop traffic, ordinary REST messages, WebSocket connections, the runtime log stream, and the full `/status` snapshot. Saving through the management console applies immediately; `.env` changes require a restart. Configure it separately to isolate permissions |
