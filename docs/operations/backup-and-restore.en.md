...
Coworker has three different backup mechanisms, matching three recovery targets: a short-term
conversation, runtime files under `data/`, or a complete instance including configuration and
external components.
...
content, which matches encrypted storage with restricted access. A backup committed to Git,
attached to an issue, or placed in a regular shared drive exposes those credentials.
...
files that are absent from the snapshot. Reproducing an exact point in time typically starts by
preserving the current state and validating the restore in an isolated directory.

> [!WARNING]
> `delete` and `backup-delete` remove runtime files below `data/`. Running them while the service
> writes removes active data as well; `--yes` skips the interactive confirmation, so scope
> verification in automation falls entirely to the caller.
...
Summary restore has the smaller impact of the two, and the common practice is to prefer it. A full
restore is typically preceded by a record of the current message count, backup filename, and time.
Neither mode restores long-term memory, Skills, or configuration.
...
volume reuse, and migration to Compose. Before the backup is verified, `docker rm -v coworker`
deletes the workspace, state, and model cache together with the anonymous volumes.
...
recovery. The container writable layer contains neither the workspace nor the state volume, so
copying it alone is not a complete backup. On a fresh host, create the state volume with
...
A drill is scheduled at least before major upgrades:
...
4. reusing a live instance identity when testing Desktop or Relay makes the drill instance
   conflict with the production one;
