...
A main model needs to meet at least the following:
...
The recommended catalog lists models statically declared to support tools. A model outside the
catalog needs its tools, images, and video support declared during first-time setup or on its
...
First-time setup and the management console are the usual routes; unattended deployments
typically use `.env` for configuration. Common fields:
...
An empty Base URL uses the Provider default. For an OpenAI-compatible gateway, the Provider type
still needs to match its actual request and response dialect. “Compatible” does not guarantee
identical tools, thinking, video, or error behavior. `openai_compatible` is the type for a
completely unknown OpenAI-compatible gateway: it has no built-in catalog, so `default_model` and
`model_capabilities` must be declared in `providers.json`.
...
Input, output, and optional cached-input prices are per million tokens. A blank cached-input
...
The usual practice is to validate each specialist before adding it to fallback; a dead Provider
first in the chain adds latency to every takeover.

## Common failures

- **Provider not registered**: a missing API key or a `DEFAULT_PROVIDER` that is not a registry
  name.
- **401/403**: common causes are the key, the Base URL, account scope, and proxies that alter
  headers.
- **404/model missing**: model IDs pass through unchanged and need to match the service's actual
  ID.
- **Tool call fails**: both model and gateway must support tool/function calling.
- **Thinking parameter fails**: unsupported levels are rejected with a 400 by the Provider; a
  `thinking_effort` matching the model's scale, or disabling thinking for that role, avoids it.
- **High latency/cost**: Runtime analytics splits main, summary, vision, bubble, subconscious, and
  long_term; pricing coverage and the Provider bill indicate which role assignments to change.
